...
/**
 * Abstract class used to contain the functions necessary for data handling through SQLite Databases
 */
public abstract class ExternalStorageTools {
    ...
    public static ArrayList<JSONObject> readEvents(){
        ...
        if(isExternalStorageReadable()){
            try{
                ...
                JSONArray contents = new JSONArray(fileContents);
                downloadedEvents = JSONTools.parseJSONArray(contents);
    ...
    public static ArrayList<JSONObject> readTeams(String event){
        ...
        if(isExternalStorageReadable()){
            try{
                ...
                JSONArray contents = new JSONArray(fileContents);
                teams = JSONTools.parseJSONArray(contents);
            } catch(FileNotFoundException e){
                e.printStackTrace();
            } catch (IOException e) {
                e.printStackTrace();
    ...
    // creates a txt file matchlist out of the ArrayList of JSONObjects from TheBlueAlliance
    public static void writeMatches(ArrayList<JSONObject> matches, String event){
        JSONArray occuringMatches = new JSONArray(matches);
        if(isExternalStorageWritable()){
            try {
                FileWriter fileWriter = new FileWriter(createFile("ScoutData/"+event,"matches.json"));
                fileWriter.write(occuringMatches.toString());
                fileWriter.flush();
                fileWriter.close();
            } catch (IOException e) {
                e.printStackTrace();
    ...
    public static ArrayList<JSONObject> readMatches(String event){
        ...
        if(isExternalStorageReadable()){
            try{
                String fileContents = "";
                String line;
                FileInputStream fileInputStream = new FileInputStream(createFile("ScoutData/"+event,"matches.json"));
                BufferedReader br = new BufferedReader(new InputStreamReader(fileInputStream));
                while((line = br.readLine())!=null)
                    fileContents += line;
                br.close();
                ...
                JSONArray contents = new JSONArray(fileContents);
                matches = JSONTools.parseJSONArray(contents);
            } catch(FileNotFoundException e){
                e.printStackTrace();
            } catch (IOException e) {
                e.printStackTrace();
    ...
    /*
    // writes a .txt data file out of MatchData to the event/device directory
